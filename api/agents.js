...
module.exports = async (req, res) => {
  ...
  try {
    const agentsRes = await supabaseRequest("agents?select=id,name,grade,created_at&order=created_at.desc&limit=100", {
      method: "GET",
      headers: { Prefer: "" }
    });
    ...
    const certsRes = await supabaseRequest("certificates?select=agent_id", {
      method: "GET",
      headers: { Prefer: "" }
    });
