...
function certWebhookUrl() {
  return process.env.DISCORD_WEBHOOK_CERT || process.env.DISCORD_WEBHOOK_URL || "";
}

function applicationWebhookUrl() {
  return process.env.DISCORD_WEBHOOK_APPLICATION || "";
}
...
async function routeSubmitApplication(req, res) {
  ...
  if (!candidateName || !candidateDiscord || !candidateMotivation) {
    return json(res, 400, { ok: false, error: "Champs obligatoires manquants." });
  }
...
async function routeLogs(req, res) {
  ...
  return json(res, 200, { ok: true, items: rows.map(r => ({
    id: r.id, action: r.action, details: r.details, createdAt: r.created_at
  }))});
}
...
async function routeAdmins(req, res) {
  ...
  const dbRes = await supabaseRequest("admin_accounts", {
    method: "POST",
    body: JSON.stringify([{ username: body.username, password, role, created_at: createdAt }])
  });
  const text = await dbRes.text();
  if (!dbRes.ok) return json(res, 500, { ok: false, error: text });
